...
// nome do evento pega aqui
$sql = "SELECT title FROM eventos WHERE id = ?";
...
<body>
	...
	<main>
<!-- header da pagina com o titulo do evento -->
			<div class="header downloads">
		<div class="container">
			<h1>Inscrição para o evento: <?php echo htmlspecialchars($eventTitle); ?></h1>
		</div>
	</div>

<!-- manda o titulo do evento pra proxima pagina pra ficar legal -->
  <form method="post" action="inscrever_enviar.php">
	<input type="hidden" name="eventTitle" value="<?php echo htmlspecialchars($eventTitle); ?>">
    <input type="hidden" name="event_id" value="<?php echo $eventId; ?>">

    <div class="mb-3">
      <label for="ra" class="form-label">RA (máx. 6 dígitos):</label>
      <input type="text" class="form-control" id="ra" name="ra" maxlength="6" pattern="\d{1,6}" required>
    </div>

    <div class="mb-3">
      <label for="cpf" class="form-label">CPF (11 dígitos):</label>
      <input type="text" class="form-control" id="cpf" name="cpf" maxlength="11" pattern="\d{11}" required>
    </div>
